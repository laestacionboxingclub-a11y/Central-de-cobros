...
3. **Carrito**: tocar un producto abre un teclado numérico grande (tipo balanza) para cargar la cantidad exacta, con atajos para pesos/cantidades comunes. Tocar la cantidad de una línea ya agregada la vuelve a abrir para corregirla. El teclado muestra el stock disponible de ese producto y no deja cargar más de lo que hay: si se pide más cantidad de la que queda, la ajusta sola al máximo disponible y avisa con un mensaje; si no queda nada, directamente no deja agregarlo. El stock que usa para este control es el mismo que ve el dueño en su panel (Paso 6), y se actualiza al toque después de cada venta para que, si se encadenan varias ventas seguidas del mismo producto, cada una vea lo que dejó la anterior. El **precio también se puede tocar y cambiar a mano** — no siempre es el de catálogo, porque muchos negocios (sobre todo los mayoristas) negocian el precio según quién compra y cuánto lleva.
4. **Cobro**: elegir método de pago (efectivo, posnet, transferencia o cuenta corriente) y confirmar. Si es efectivo, aparece un campo "¿Con cuánto paga?" — al escribirlo calcula el vuelto al toque, y ese vuelto queda impreso en el ticket también. Al cobrar se ve una animación de "ticket saliendo de la impresora" — con el contenido real del comprobante impreso ahí mismo (no un cartel en blanco) y un cabezal verde que "escanea" el papel — antes de mostrar el comprobante final. Hay animaciones chicas en varios lugares más (el carrito, los botones, el teclado) para que se sienta más vivo; todas respetan la preferencia "reducir movimiento" del sistema operativo, para quien la tenga activada.
...
- **Productos**: crear, editar, activar/desactivar. Cada uno con nombre, precio, unidad (kg, unidad, cajón, bulto, saco, etc. — es texto libre, no una lista cerrada) y un "stock mínimo" opcional. Si el stock actual de un producto activo cae por debajo de ese mínimo, aparece un aviso arriba de la lista — esa es la alerta de stock bajo. También se puede **ajustar el precio a varios productos a la vez**: se seleccionan con los checkboxes (o "Seleccionar todos") y se les aplica un porcentaje — positivo para subir, negativo para bajar — de una sola vez, en vez de entrar producto por producto.
  ...
  Un producto puede además tener un **atajo de bolsa/cajón entero** (tildando "También se vende entero por bolsa/cajón"): se le carga cuántos kilos tiene esa bolsa y a qué precio se vende entera (no siempre es exactamente kilos × precio por kilo — vender la bolsa completa suele tener su propio precio). El stock sigue llevándose siempre en la unidad de base (kg) — esto no crea un producto separado, es solo un atajo de carga rápida: en la Caja aparece un botón "1 bolsa (25 kg) — $10.000" que carga esa cantidad y ajusta el precio por kg para que el total cierre justo en ese precio.
...
### Pedidos (comandas) y rol vendedor

Pensado para el caso de un local grande donde varias personas atienden en el piso mientras arman el pedido de cada cliente (bultos, cajones, cantidad grande), y recién después esa persona se acerca a una caja a pagar y retirar.

- **Rol nuevo: vendedor.** No maneja caja ni efectivo — solo puede tomar pedidos. Entra a la misma app de Caja con su propio usuario (se lo da de alta el dueño, igual que a un cajero) desde su propio celular — no hace falta comprarle una tablet ni instalarle nada, es una página web común.
- **Tomar un pedido**: el vendedor elige productos, cantidad y el **precio que haya negociado** con el cliente (no el de catálogo — acá tampoco hay precios fijos), le pone un nombre de referencia si quiere, y confirma. Eso genera un **número de pedido** y lo manda a imprimir en una impresora térmica Bluetooth portátil.
- **Cobrar un pedido**: en la Caja, el botón "Pedidos" del encabezado abre la lista de pedidos esperando cobro — se busca por número o nombre, se elige, y el carrito se completa solo con lo que pidió esa persona. De ahí en adelante es el cobro de siempre (cualquier método de pago, incluida cuenta corriente); al confirmar, el pedido queda marcado como cobrado.
- Migración [`0008_pedidos.sql`](supabase/migrations/0008_pedidos.sql): tablas `pedidos` y `pedido_items`, más el rol `vendedor` sumado a los permitidos. Esta migración también corrige un bug: al agregar cuenta corriente como método de pago (migración anterior) nunca se había actualizado la restricción de la base que validaba los métodos de pago permitidos — quedó arreglado acá.

**Sobre la impresión Bluetooth**: el código para imprimir el ticket ya está escrito (`apps/caja/src/pedidos/imprimirPedido.ts`), pero usa un protocolo genérico típico de las impresoras térmicas Bluetooth chinas más baratas — **no está probado contra una impresora real todavía**. Cuando llegue la primera, hay que probarlo y ajustar los identificadores del protocolo si hace falta (está todo comentado en ese archivo, es un cambio chico si el protocolo no coincide). Mientras tanto, si la impresión falla, el pedido igual queda guardado y el número se le puede mostrar al cliente en la pantalla.
